// Mock local de Supabase para desarrollo sin conexión
// Usuarios de prueba:
...
const STORAGE_KEY = 'tickets_ft_mock'
const SESSION_KEY = 'tickets_ft_session'
  ...
  tickets: [
    {
      ...
      estado: 'abierto', sucursal_id: 'suc1', usuario_id: 'user2',
    ...
    },
    {
      ...
      tipo_falla: null,
      estado: 'en_proceso', sucursal_id: 'suc2', usuario_id: 'user2',
    ...
    },
    {
      ...
      estado: 'resuelto', sucursal_id: 'suc1', usuario_id: 'user2',
      created_at: '2025-03-22T08:00:00Z', updated_at: '2025-03-23T14:00:00Z'
    }
  ],
  comentarios: [
    {
      id: 'com1', ticket_id: 'tk2', usuario_id: 'user3',
      contenido: 'Revisando el problema, parece ser un tema de permisos en el servidor.',
      created_at: '2025-03-21T11:00:00Z'
    }
  ],
  historial_tickets: [
    { id: 'h1', ticket_id: 'tk1', usuario_id: 'user2', accion: 'Ticket creado', detalle: 'Reporte R-000001 creado por María García', created_at: '2025-03-20T10:00:00Z' },
    { id: 'h2', ticket_id: 'tk2', usuario_id: 'user2', accion: 'Ticket creado', detalle: 'Reporte R-000002 creado por María García', created_at: '2025-03-21T09:30:00Z' },
    { id: 'h3', ticket_id: 'tk2', usuario_id: 'user3', accion: 'Estado actualizado', detalle: 'Estado cambiado a: En Proceso', created_at: '2025-03-21T11:00:00Z' },
    { id: 'h4', ticket_id: 'tk3', usuario_id: 'user2', accion: 'Ticket creado', detalle: 'Ticket TKT-2025-00003 creado por María García', created_at: '2025-03-22T08:00:00Z' },
    { id: 'h5', ticket_id: 'tk3', usuario_id: 'user3', accion: 'Estado actualizado', detalle: 'Estado cambiado a: Resuelto', created_at: '2025-03-23T14:00:00Z' }
  ]
}

// ---------- Persistencia ----------
const DATA_VERSION = 'v3' // Incrementar si cambian los datos iniciales
...
// ---------- Joins ----------
function applyJoins(row, tableName, store) {
  const r = { ...row }
  if (tableName === 'tickets') {
    r.sucursales = store.sucursales.find(s => s.id === row.sucursal_id) || null
    r.profiles   = store.profiles.find(p => p.id === row.usuario_id) || null
  }
  if (tableName === 'comentarios') {
    const p = store.profiles.find(p => p.id === row.usuario_id) || null
    r.profiles = p
  }
...
class QueryBuilder {
  ...
  then(resolve, reject) {
    return Promise.resolve(this._run(false)).then(resolve, reject)
  }
  ...
  _run(single) {
    ...
    if (this._op === 'insert') {
      ...
      store[this._table] = [...(table || []), newRow]
      saveStore(store)
      // re-fetch with joins for response
      const withJoin = applyJoins(newRow, this._table, store)
      return single ? { data: withJoin, error: null } : { data: [withJoin], error: null }
    }

    if (this._op === 'update') {
      let updated = null
      store[this._table] = (table || []).map(row => {
        if (this._filters.every(f => row[f.col] == f.val)) {
          updated = { ...row, ...this._payload, updated_at: new Date().toISOString() }
          return updated
        }
        return row
      })
    ...
    if (this._op === 'delete') {
      store[this._table] = (table || []).filter(row =>
    ...
    rows = rows.map(r => applyJoins(r, this._table, store))
...
const auth = {
  ...
  signInWithPassword: ({ email, password }) => {
    const store = getStore()
    const authUser = store.users_auth.find(u => u.email === email && u.password === password)
    if (!authUser) {
      return Promise.resolve({ data: null, error: { message: 'Credenciales incorrectas' } })
    }
    const user = { id: authUser.id, email: authUser.email }
    const session = { user, access_token: 'mock-token-' + authUser.id }
...
export const supabase = {
  ...
  rpc: (fn) => {
    if (fn === 'generate_ticket_folio') {
      _folioSeq++
      const folio = `R-${String(_folioSeq).padStart(6, '0')}`
      return Promise.resolve({ data: folio, error: null })
    }
  ...
  functions: {
    invoke: (name, { body } = {}) => {
      if (name === 'crear-usuario') {
        const store = getStore()
        const newId = genId()
        store.users_auth.push({ id: newId, email: body.email, password: body.password })
        store.profiles.push({
          id: newId, nombre: body.nombre, rol: body.rol,
          sucursal_id: body.sucursal_id || null, email: body.email
        })
        saveStore(store)
