...
router.get('/', (req, res) => {
  try {
    const rows = db.prepare('SELECT categoria, tecnico_ids, updated_at FROM reglas_asignacion').all()
    ...
    const reglas = rows.map(row => {
      ...
      const tecnicos = ids.length > 0
        ? db.prepare(`SELECT id, nombre FROM profiles WHERE id IN (${ids.map(() => '?').join(',')})`)
            .all(...ids)
        : []
      return { categoria: row.categoria, tecnico_ids: ids, tecnicos, updated_at: row.updated_at }
    })
...
router.put('/:categoria', (req, res) => {
  ...
  try {
    db.prepare(`
      INSERT INTO reglas_asignacion (categoria, tecnico_ids, updated_at)
      VALUES (?, ?, datetime('now'))
      ON CONFLICT(categoria) DO UPDATE SET tecnico_ids = excluded.tecnico_ids, updated_at = excluded.updated_at
    `).run(categoria, JSON.stringify(tecnico_ids))

    const row = db.prepare('SELECT categoria, tecnico_ids, updated_at FROM reglas_asignacion WHERE categoria = ?').get(categoria)
    const ids = JSON.parse(row.tecnico_ids)
    const tecnicos = db.prepare(`SELECT id, nombre FROM profiles WHERE id IN (${ids.map(() => '?').join(',')})`).all(...ids)

    res.json({ categoria: row.categoria, tecnico_ids: ids, tecnicos, updated_at: row.updated_at })
  } catch (err) {
